...
ssl pinning单向校验，本地校验，失败提示异常(网络请求未发出去)。
用算法助手(JustTrustMe)模块，hook，或Frida底层绕过证书抓包。

安卓防止检测root不让打开app
>隐藏root，hook用隐藏应用列表模块。
